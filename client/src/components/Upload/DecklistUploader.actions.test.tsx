...
vi.mock("@/helpers/dbUtils", () => ({
  ...
  moveMultiFaceCardsToEnd: (...args: unknown[]) =>
    mocks.moveMultiFaceCardsToEnd(...args),
  checkMultiFaceCardsHaveCorrectBack: (...args: unknown[]) =>
    mocks.checkMultiFaceCardsHaveCorrectBack(...args),
  countBasicLandsToRemove: (...args: unknown[]) =>
    mocks.countBasicLandsToRemove(...args),
  removeBasicLandsFromProject: (...args: unknown[]) =>
    mocks.removeBasicLandsFromProject(...args),
}));

vi.mock("@/helpers/tokenImportHelper", () => ({
  handleManualTokenImport: (...args: unknown[]) =>
    mocks.handleManualTokenImport(...args),
  handleManualTwoSidedTokenImport: (...args: unknown[]) =>
    mocks.handleManualTwoSidedTokenImport(...args),
}));
...
vi.mock("@/store", () => {
  const useSettingsStore = Object.assign(
    vi.fn(
      (
        selector?: (state: {
          preferredArtSource: typeof mocks.preferredArtSource;
          setSortBy: typeof mocks.setSortBy;
        }) => unknown
      ) => {
        const state = {
          preferredArtSource: mocks.preferredArtSource,
  ...
  return {
    ...
    useCardsStore: (
      selector: (state: {
        clearAllCardsAndImages: typeof mocks.clearAllCardsAndImages;
      }) => unknown
    ) => selector({ clearAllCardsAndImages: mocks.clearAllCardsAndImages }),
  };
});
...
vi.mock("@/store/loading", () => ({
  useLoadingStore: (
    selector: (state: {
      setLoadingTask: typeof mocks.setLoadingTask;
    }) => unknown
  ) => selector({ setLoadingTask: mocks.setLoadingTask }),
}));
    ...
    vi.fn(
      (
        selector?: (state: {
          ...
          removeToast: typeof mocks.removeToast;
        }) => unknown
      ) => {
        const state = {
...
vi.mock("../ArtworkModal", () => ({
  AdvancedSearch: ({
    ...
    <div data-testid="advanced-search">
      <button
        onClick={() =>
          onSelectCard("Forest (Ukiyo)", "https://example.test/forest.jpg")
        }
      >
        Select MPC Card
      </button>
      <button
        onClick={() =>
          onSelectCard("Counterspell", undefined, { set: "mh2", number: "267" })
        }
      >
        Select Scryfall Card
      </button>
...
function renderDecklist(
  cardCount = 1,
  props: Partial<React.ComponentProps<typeof DecklistUploader>> = {}
) {
  return render(<DecklistUploader cardCount={cardCount} {...props} />);
}

function typeDeck(text: string) {
  fireEvent.change(screen.getByPlaceholderText(/1x Sol Ring/), {
    target: { value: text },
  });
}
...
describe("DecklistUploader action branches", () => {
  beforeEach(() => {
    ...
    mocks.moveMultiFaceCardsToEnd.mockResolvedValue({
      multiFaceSlots: 2,
      updatedSlots: 1,
    });
    mocks.checkMultiFaceCardsHaveCorrectBack.mockResolvedValue({
      multiFace: 2,
      checked: 2,
      fixed: 0,
      skipped: 0,
    ...
    mocks.handleManualTwoSidedTokenImport.mockResolvedValue({
  ...
  it("submits deck text with preferred art fallback, clears text on completion, and supports Ctrl+Enter", async () => {
    ...
    await waitFor(() =>
      expect(mocks.processCards).toHaveBeenCalledWith([
        { name: "Island", quantity: 2, sourcePreference: "scryfall" },
      ])
    );
    expect(onUploadComplete).toHaveBeenCalledTimes(2);
    expect(
      (screen.getByPlaceholderText(/1x Sol Ring/) as HTMLTextAreaElement).value
    ).toBe("");

    mocks.parseDeckList.mockReturnValue([
      { name: "Swamp", quantity: 1, sourcePreference: "mpc" },
    ]);
    typeDeck("1 Swamp");
    fireEvent.keyDown(screen.getByPlaceholderText(/1x Sol Ring/), {
      key: "Enter",
      ctrlKey: true,
    });
    await waitFor(() =>
  ...
  it("adds advanced-search MPC and Scryfall selections with the right import intents", async () => {
    ...
    await waitFor(() =>
      expect(mocks.addRemoteImage).toHaveBeenCalledWith(
        ["https://example.test/forest.jpg"],
        1
      )
    );
    expect(mocks.processCards).toHaveBeenCalledWith([
  ...
  it("reports multi-face move outcomes and failures", async () => {
    ...
    await waitFor(() =>
      expect(mocks.showInfoToast).toHaveBeenCalledWith(
        "Moved 2 multi-face cards to the end."
      )
    );
    expect(mocks.setSortBy).toHaveBeenCalledWith("manual");

    mocks.moveMultiFaceCardsToEnd.mockResolvedValueOnce({
      multiFaceSlots: 0,
      updatedSlots: 0,
    });
    fireEvent.click(screen.getByText("Move Multi-Face Cards To End"));
    ...
    await waitFor(() =>
      expect(mocks.showInfoToast).toHaveBeenCalledWith(
        "Multi-face cards are already at the end."
      )
    );

    mocks.moveMultiFaceCardsToEnd.mockRejectedValueOnce(
      new Error("reorder failed")
    );
    fireEvent.click(screen.getByText("Move Multi-Face Cards To End"));
    await waitFor(() =>
      expect(mocks.showErrorToast).toHaveBeenCalledWith("reorder failed")
    );
  ...
  it("checks multi-face backs and reports clean, fixed, none, and failure states", async () => {
    renderDecklist(1);
    fireEvent.click(
      screen.getByText("Check Multi-Face Cards Have Correct Back")
    );
    await waitFor(() =>
      expect(mocks.showInfoToast).toHaveBeenCalledWith(
        "Checked 2 multi-face cards: all backs look OK. (Checked=2, Fixed=0, Skipped=0, Errors=0)"
      )
    );

    mocks.checkMultiFaceCardsHaveCorrectBack.mockResolvedValueOnce({
      multiFace: 1,
      checked: 1,
      fixed: 1,
      skipped: 0,
    ...
    await waitFor(() =>
      expect(mocks.showInfoToast).toHaveBeenCalledWith(
        "Multi-face back check complete: fixed 1/1 broken card. (Checked=1, Fixed=1, Skipped=0, Errors=0)"
      )
    );
    ...
    await waitFor(() =>
      expect(mocks.showInfoToast).toHaveBeenCalledWith(
        "No multi-face cards detected."
      )
    );

    mocks.checkMultiFaceCardsHaveCorrectBack.mockRejectedValueOnce(
    ...
    await waitFor(() =>
      expect(mocks.showErrorToast).toHaveBeenCalledWith("check failed")
    );
  });
  ...
  it("removes basics with selected options and handles no-op and failure results", async () => {
    ...
    await waitFor(() =>
      expect(mocks.removeBasicLandsFromProject).toHaveBeenCalledWith(
        "project-1",
        { includeWastes: false, includeSnowCovered: true }
      )
    );
    expect(mocks.showInfoToast).toHaveBeenCalledWith("Removed 2 basic lands.");

    mocks.removeBasicLandsFromProject.mockResolvedValueOnce({
      removedBasics: 0,
    });
    fireEvent.click(screen.getByText("Remove All Basic Lands"));
    fireEvent.click(screen.getByText("Remove"));
    await waitFor(() =>
      expect(mocks.showInfoToast).toHaveBeenCalledWith(
        "No basic lands found to remove."
      )
    );
  ...
  it("clears cards immediately when empty and through confirmation when populated", async () => {
    ...
    await waitFor(() =>
      expect(mocks.clearAllCardsAndImages).toHaveBeenCalledTimes(1)
    );
    ...
    await waitFor(() =>
      expect(mocks.clearAllCardsAndImages).toHaveBeenCalledTimes(2)
    );

    mocks.clearAllCardsAndImages.mockRejectedValueOnce(
      new Error("clear failed")
    );
    mocks.cardCount.mockResolvedValueOnce(0);
    fireEvent.click(screen.getByText("Clear Cards"));
    await waitFor(() =>
      expect(mocks.showErrorToast).toHaveBeenCalledWith("clear failed")
    );
  });

  it("runs token imports, no-token modal, paired summaries, and error fallbacks", async () => {
    renderDecklist(1);
    mocks.handleManualTokenImport.mockImplementationOnce(async (options) =>
      options.onNoTokens()
    );
    ...
    mocks.handleManualTokenImport.mockRejectedValueOnce(
      new Error("token failed")
    );
    fireEvent.click(screen.getByText("Add Associated Tokens"));
    await waitFor(() =>
      expect(mocks.showErrorToast).toHaveBeenCalledWith("token failed")
    );

    fireEvent.click(screen.getByText("Add two sided associated tokens"));
    await waitFor(() =>
      expect(mocks.showInfoToast).toHaveBeenCalledWith(
        "Added 2 two sided associated tokens."
      )
    );

    mocks.handleManualTwoSidedTokenImport.mockResolvedValueOnce({
      importedTokenCount: 3,
      pairedTokenCount: 1,
    ...
    await waitFor(() =>
      expect(mocks.showInfoToast).toHaveBeenCalledWith(
        "Added 1 two sided associated token; 2 could not be paired without matching itself."
      )
    );
    ...
    await waitFor(() =>
      expect(mocks.showInfoToast).toHaveBeenCalledWith(
        "At least two different token arts are needed to make two sided associated tokens."
      )
    );

    mocks.handleManualTwoSidedTokenImport.mockRejectedValueOnce(
      new Error("two-side failed")
    );
    fireEvent.click(screen.getByText("Add two sided associated tokens"));
    await waitFor(() =>
      expect(mocks.showErrorToast).toHaveBeenCalledWith("two-side failed")
    );
  });

  it("closes auxiliary modals and toggles the remaining modal-only controls", async () => {
    renderDecklist(1);
    ...
    await waitFor(() =>
      expect(screen.queryByText("Confirm Clear Cards")).toBeNull()
    );
  });
});
