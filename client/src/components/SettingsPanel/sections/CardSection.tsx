...
export function CardSection() {
  ...
  const useCustomBackOffset = useSettingsStore(
    (state) => state.useCustomBackOffset
  );
  const cardBackPositionX = useSettingsStore(
    (state) => state.cardBackPositionX
  );
  ...
  const setUseCustomBackOffset = useSettingsStore(
    (state) => state.setUseCustomBackOffset
  );
  const setCardBackPositionX = useSettingsStore(
    (state) => state.setCardBackPositionX
  );
  ...
  const cardBackPositionXInput = usePositionInput(
    cardBackPositionX,
    setCardBackPositionX
  );
  const cardBackPositionYInput = usePositionInput(
    cardBackPositionY,
          ...
          <Button
            ...
            onClick={() =>
              openCalibrationModal({
                cardUuid: "settings-calibration",
                card: {
                  uuid: "settings-calibration",
            ...
            className="flex-1 gap-2"
            data-testid="open-mpc-calibration-settings"
          >
            <ScanLine className="h-5 w-5" />
            MPC Calibration Harness
          </Button>
          <AutoTooltip content="Open the MPC calibration harness to import a frozen fixture, capture expected picks, and replay the automatic chooser locally." />
        </div>
      </div>
