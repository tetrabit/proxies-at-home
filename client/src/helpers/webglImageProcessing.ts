...
function trackContextCreation(source: string): number {
  ...
  webglLog(
    `Context CREATED #${id} by ${source} (active: ${webglContextCount})`
  );
  return id;
}
...
function trackContextRelease(id: number, source: string) {
  webglContextCount--;
  webglLog(
    `Context RELEASED #${id} by ${source} (active: ${webglContextCount})`
  );
}
...
class WebGLContextManager {
  ...
  getContext(
    width: number,
    height: number
  ): { canvas: OffscreenCanvas; gl: WebGL2RenderingContext; isNew: boolean } {
    ...
    if (this.canvas && this.gl && !this.isContextLost) {
      ...
      if (this.gl.isContextLost()) {
        webglLog(
          `Context for ${this.purpose} was silently lost, recreating...`
        );
    ...
    webglLog(
      `Context manager created new context for ${this.purpose}: ${width}x${height}`
    );
...
function runJfaSteps(
  ...
  gl.uniform2f(
    gl.getUniformLocation(stepProgram, "u_resolution"),
    width,
    height
  );
...
export async function generateBleedCanvasWebGL(
  ...
  const bleedWidthMm =
    (opts?.unit ?? "mm") === "in" ? bleedWidth * 25.4 : bleedWidth;
  ...
  const { canvas, gl, isNew } = bleedGenContextManager.getContext(
    finalWidth,
    finalHeight
  );

  webglLog(
    `Bleed generation: ${finalWidth}x${finalHeight}, bleed=${bleedWidth}${opts?.unit ?? "mm"}, reused=${!isNew}`
  );
  ...
  const inputBleedMm =
    (opts?.unit ?? "mm") === "in" ? inputBleedRaw * 25.4 : inputBleedRaw;
  const { bleedPxX: inputBleedPxX, bleedPxY: inputBleedPxY } =
    deriveSourceBleedPixelsFromGeometry(img.width, img.height, inputBleedMm);
  ...
  if (inputBleedMm > 0) {
    ...
    if (inputContentWidth > 0 && inputContentHeight > 0) {
      ...
      const inputContentCenterX =
        (inputContentWidth / 2 + inputBleedPxX) * scale;
      const inputContentCenterY =
        (inputContentHeight / 2 + inputBleedPxY) * scale;
  ...
  gl.uniform2f(
    gl.getUniformLocation(progs.init, "u_resolution"),
    finalWidth,
    finalHeight
  );
  gl.uniform2f(
    gl.getUniformLocation(progs.init, "u_imageSize"),
    targetCardWidth,
    targetCardHeight
  );
  gl.uniform2f(gl.getUniformLocation(progs.init, "u_offset"), bleed, bleed);
  gl.uniform2f(
    gl.getUniformLocation(progs.init, "u_srcImageSize"),
    img.width,
    img.height
  );
  gl.uniform2f(
    gl.getUniformLocation(progs.init, "u_srcOffset"),
    sourceOffsetX,
    sourceOffsetY
  );
  ...
  // --- PASS 2: JFA STEPS ---
  const finalSeedTex = runJfaSteps(
    gl,
    progs.step,
    texA,
    texB,
  ...
  gl.uniform2f(
    gl.getUniformLocation(progs.final, "u_resolution"),
    finalWidth,
    finalHeight
  );
  gl.uniform2f(
    gl.getUniformLocation(progs.final, "u_imageSize"),
    targetCardWidth,
    targetCardHeight
  );
  gl.uniform2f(gl.getUniformLocation(progs.final, "u_offset"), bleed, bleed);
  gl.uniform1i(
    gl.getUniformLocation(progs.final, "u_darkenMode"),
    darkenModeInt
  );
  gl.uniform1f(
    gl.getUniformLocation(progs.final, "u_darknessFactor"),
    darknessFactor
  );
  // Explicitly set darken params using opts
  gl.uniform1f(
    gl.getUniformLocation(progs.final, "u_darkenThreshold"),
    opts.darkenThreshold ?? 30
  );
  gl.uniform1f(
    gl.getUniformLocation(progs.final, "u_darkenContrast"),
    opts.darkenContrast ?? 2.0
  );
  gl.uniform1f(
    gl.getUniformLocation(progs.final, "u_darkenEdgeWidth"),
    opts.darkenEdgeWidth ?? 0.1
  );
  gl.uniform1f(
    gl.getUniformLocation(progs.final, "u_darkenAmount"),
    opts.darkenAmount ?? 1.0
  );
  gl.uniform1f(
    gl.getUniformLocation(progs.final, "u_darkenBrightness"),
    opts.darkenBrightness ?? -50
  );

  gl.uniform2f(
    gl.getUniformLocation(progs.final, "u_srcImageSize"),
    img.width,
    img.height
  );
  gl.uniform2f(
    gl.getUniformLocation(progs.final, "u_srcOffset"),
    sourceOffsetX,
    sourceOffsetY
  );
...
export async function processCardImageWebGL(
  ...
  const inputGeometry = getCardPixelDimensionsForBleed(
    Math.min(inputHasBleedMm, totalBleedMm),
    exportDpi
  );
  const targetGeometry = getCardPixelDimensionsForBleed(
    totalBleedMm,
    exportDpi
  );
  const displayGeometry = getCardPixelDimensionsForBleed(
    totalBleedMm,
    displayDpi
  );
  ...
  const finalWidth = targetGeometry.width;
  const finalHeight = targetGeometry.height;
  const additionalBleedPxX = Math.max(
    0,
    Math.round((finalWidth - inputWidth) / 2)
  );
  const additionalBleedPxY = Math.max(
    0,
    Math.round((finalHeight - inputHeight) / 2)
  );

  // Get or create WebGL context using context manager (reuses if available)
  const { canvas, gl, isNew } = jfaContextManager.getContext(
    finalWidth,
    finalHeight
  );

  webglLog(
    `JFA processing: ${finalWidth}x${finalHeight}, additionalBleed=${additionalBleedMm}mm, reused=${!isNew}`
  );
  ...
  gl.uniform2f(
    gl.getUniformLocation(progs.init, "u_offset"),
    additionalBleedPxX,
    additionalBleedPxY
  );
  gl.uniform2f(
  ...
  async function renderFinalAndExtract(
    ...
    glCtx.uniform2f(
      glCtx.getUniformLocation(progs.final, "u_resolution"),
      finalWidth,
      finalHeight
    );
    glCtx.uniform2f(
      glCtx.getUniformLocation(progs.final, "u_imageSize"),
      inputWidth,
      inputHeight
    );
    glCtx.uniform2f(
      glCtx.getUniformLocation(progs.final, "u_offset"),
      additionalBleedPxX,
      additionalBleedPxY
    );
    glCtx.uniform1i(
      glCtx.getUniformLocation(progs.final, "u_darkenMode"),
      darkenMode
    );
    glCtx.uniform1f(
      glCtx.getUniformLocation(progs.final, "u_darknessFactor"),
      darknessFactor
    );
    // Explicitly set darken params
    glCtx.uniform1f(
      glCtx.getUniformLocation(progs.final, "u_darkenThreshold"),
      opts?.darkenThreshold ?? 30
    );
    glCtx.uniform1f(
      glCtx.getUniformLocation(progs.final, "u_darkenContrast"),
      opts?.darkenContrast ?? 2.0
    );
    glCtx.uniform1f(
      glCtx.getUniformLocation(progs.final, "u_darkenEdgeWidth"),
      opts?.darkenEdgeWidth ?? 0.1
    );
    glCtx.uniform1f(
      glCtx.getUniformLocation(progs.final, "u_darkenAmount"),
      opts?.darkenAmount ?? 1.0
    );
    glCtx.uniform1f(
      glCtx.getUniformLocation(progs.final, "u_darkenBrightness"),
      opts?.darkenBrightness ?? -50
    );

    glCtx.uniform2f(
      glCtx.getUniformLocation(progs.final, "u_srcImageSize"),
      img.width,
      img.height
    );
    glCtx.uniform2f(
      glCtx.getUniformLocation(progs.final, "u_srcOffset"),
      sourceOffsetX,
      sourceOffsetY
    );
    glCtx.uniform2f(
      glCtx.getUniformLocation(progs.final, "u_scale"),
      scaleX,
      scaleY
    );
    ...
    const displayBlob = await lowResCanvas.convertToBlob({
      type: "image/webp",
      quality: 0.9,
    });
...
export async function processExistingBleedWebGL(
  ...
  const modesToGenerate =
    currentDarkenMode === 0 ? [0] : [0, currentDarkenMode];
    ...
    for (const mode of modesToGenerate) {
      ...
      blobs.set(
        ...
        await canvas.convertToBlob({
          type: mimeType,
          quality: mimeType === "image/webp" ? 0.9 : undefined,
        })
      );
    }
  ...
  const exportBlobs = await renderSelectedModes(
    exportWidth,
    exportHeight,
    "image/png"
  );
  const displayBlobs = await renderSelectedModes(
    displayWidth,
    displayHeight,
    "image/webp"
  );
...
export async function renderBleedCanvasDirect(
  ...
  const contextManager =
    !opts.mimeType || isExport ? exportContextManager : displayContextManager;
  ...
  gl.uniform1f(
    gl.getUniformLocation(program, "u_darknessFactor"),
    darknessFactor
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_darkenThreshold"),
    opts.darkenThreshold ?? 30
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_darkenContrast"),
    opts.darkenContrast ?? 2.0
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_darkenEdgeWidth"),
    opts.darkenEdgeWidth ?? 0.1
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_darkenAmount"),
    opts.darkenAmount ?? 1.0
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_darkenBrightness"),
    opts.darkenBrightness ?? -50
  );
  gl.uniform1i(
    gl.getUniformLocation(program, "u_darkenMode"),
    opts.darkenMode ?? 0
  );
...
// Narrow test seam for deterministic Vitest coverage of private WebGL helpers.
// Production callers should use the exported processing functions above.
export const __webglImageProcessingTestInternals = {
  WebGLContextManager,
  computeDarknessFactor,
  initWebGLPrograms,
  runJfaSteps,
