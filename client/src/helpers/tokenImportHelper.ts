 ...
 */
export async function handleManualTokenImport(
  options: Omit<AutoTokenOptions, "force"> = {}
) {
  const { silent = false } = options;
 ...
 */
export async function handleManualTwoSidedTokenImport(
  options: Omit<AutoTokenOptions, "force"> = {}
): Promise<TwoSidedTokenImportResult> {
  const { silent = false } = options;
  ...
  try {
    ...
    const afterCards = await db.cards
      ...
      .equals(projectId)
      .toArray();
    const importedTokens = getImportedTokenCards(
      afterCards,
      tokenIntents,
      beforeUuids
    );
...
function getExistingAssociatedTokenCards(
  cards: CardOption[]
): PairableTokenCard[] {
...
function isTokenCard(card: Pick<CardOption, "isToken" | "type_line">): boolean {
  return (
    card.isToken === true ||
    card.type_line?.toLowerCase().includes("token") === true
  );
}

async function applyTwoSidedTokenBacks(
  pairs: TwoSidedTokenPair[]
): Promise<void> {
  ...
  await db.transaction("rw", db.cards, db.images, async () => {
    ...
    for (const card of projectCards) {
      if (!card.linkedFrontId || !frontUuidSet.has(card.linkedFrontId))
        continue;
    ...
    for (const { front, back } of pairs) {
      ...
      if (currentFront.linkedBackId) {
        ...
        if (
          linkedBack &&
          !existingBacks.some((candidate) => candidate.uuid === linkedBack.uuid)
        ) {
...
async function applyImageRefDeltas(deltas: Map<string, number>): Promise<void> {
  const imageIds = Array.from(deltas.keys()).filter(
    (imageId) => deltas.get(imageId) !== 0
  );
  ...
  for (let i = 0; i < imageIds.length; i++) {
    ...
    // v8 ignore next -- @preserve imageIds is derived from deltas.keys(), so the fallback is defensive only.
...
export function createShuffledTwoSidedTokenPairs(
  cards: PairableTokenCard[]
): TwoSidedTokenPair[] {
  ...
  for (let attempt = 0; attempt < 500; attempt++) {
    ...
    if (
      cards.every((front, index) => canUseTokenAsBack(front, shuffled[index]))
    ) {
...
function findTwoSidedTokenPairs(
  fronts: PairableTokenCard[],
  backs: PairableTokenCard[]
): TwoSidedTokenPair[] {
...
function getIntentIdentityKey(intent: ImportIntent): string {
  return intent.scryfallId
    ? `id:${intent.scryfallId}`
    : `name:${normalizeTokenName(intent.name)}`;
}
...
function getCardIdentityKey(
  card: Pick<CardOption, "name" | "scryfall_id">
): string {
  return card.scryfall_id
    ? `id:${card.scryfall_id}`
    : `name:${normalizeTokenName(card.name)}`;
}
