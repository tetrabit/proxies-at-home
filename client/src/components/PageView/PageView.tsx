...
import { useMpcUpgradeModalStore } from "@/store/mpcUpgradeModal";
...
import PixiVirtualCanvas, {
  ...
  type PageLayoutInfo,
} from "../PixiPage/PixiVirtualCanvas";
...
export function PageView({
  ...
  const showGuideLinesOnBackCards = useSettingsStore(
    (s) => s.showGuideLinesOnBackCards
  );
  ...
  const allCardUuids = useMemo(
    () => cards.filter((c) => !c.linkedFrontId).map((c) => c.uuid),
    [cards]
  );
  ...
  const [renderedCardUuids, setRenderedCardUuids] = useState<Set<string>>(
    new Set()
  );
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        useArtworkModalStore.getState().open ||
        useCardEditorModalStore.getState().open ||
        useMpcUpgradeModalStore.getState().open
      ) {
        return;
      }
  ...
  const handleDragStart = useCallback(
    (event: DragStartEvent) => {
      ...
      const isMultiSelect =
        selectionStore.selectedCards.has(cardUuid) &&
        selectionStore.selectedCards.size > 1;

      if (isMultiSelect) {
        const draggedCards = localCards.filter((c) =>
          selectionStore.selectedCards.has(c.uuid)
        );
        ...
        const remainingCards = localCards.filter(
          (c) => !selectionStore.selectedCards.has(c.uuid)
        );
        const leaderOriginalIndex = localCards.findIndex(
          (c) => c.uuid === cardUuid
        );
  ...
  const handleDragEnd = useCallback(async (event: DragEndEvent) => {
    ...
    if (multiDragState.current.isMultiDrag) {
      ...
      const leaderIndex = localCardsRef.current.findIndex(
        (c) => c.uuid === leaderId
      );
      if (leaderIndex === -1) {
        setLocalCards(multiDragState.current.originalLocalCards);
      ...
      const cardsWithoutLeader = localCardsRef.current.filter(
        (c) => c.uuid !== leaderId
      );
      const newLocalCards = [
    ...
    // Single Card Logic
    const oldIndex = localCardsRef.current.findIndex(
      (c) => c.uuid === active.id
    );
    ...
    if (
      Math.abs(newOrder - (prevCard?.order || 0)) < 0.001 ||
      Math.abs(newOrder - (nextCard?.order || 0)) < 0.001
    ) {
  ...
  const totalContentHeight =
    pageCount * pageHeightPx + (pageCount + 1) * PAGE_GAP_PX + topPaddingPx;
  ...
  const pixiPages = useMemo((): PageLayoutInfo[] => {
    return Array.from({ length: pageCount }, (_, i) => ({
      ...
      pageYOffset:
        topPaddingPx + PAGE_GAP_PX + i * (pageHeightPx + PAGE_GAP_PX),
    }));
  ...
  const frontCardOverridesKey = localCards
    .map(
      (c) =>
        `${c.overrides?.brightness}:${c.overrides?.contrast}:${c.overrides?.saturation}:${c.overrides?.holoEffect}:${c.overrides?.holoAnimation}`
    )
    .join(",");
  const backCardOverridesKey = Array.from(backCardMap.values())
    .map(
      (bc) =>
        `${bc.overrides?.brightness}:${bc.overrides?.contrast}:${bc.overrides?.saturation}:${bc.overrides?.holoEffect}:${bc.overrides?.holoAnimation}`
    )
    .join(",");
  ...
  const globalPixiCards = useMemo((): CardWithGlobalLayout[] => {
    ...
    const gridWidthMm =
      columns * fixedCardWidthMm + (columns - 1) * cardSpacingMm;
    ...
    pages.forEach((page, pageIndex) => {
      ...
      const isPageFullyFlipped =
        page.length > 0 && page.every((c) => flippedCards.has(c.uuid));
      const useBackOffsets = useCustomBackOffset && isPageFullyFlipped;

      const effectiveCardPositionX = useBackOffsets
        ? cardBackPositionX
        : cardPositionX;
      const effectiveCardPositionY = useBackOffsets
        ? cardBackPositionY
        : cardPositionY;
      ...
      page.forEach((card, index) => {
        ...
        const cellYMm =
          gridStartYMm + row * (fixedCardHeightMm + cardSpacingMm);
        ...
        result.push({
          ...
          globalY:
            topPaddingPx +
            PAGE_GAP_PX +
            pageIndex * (pageHeightPx + PAGE_GAP_PX) +
            yMm * MM_TO_PX,
    ...
    fixedCardHeightMm,
    topPaddingPx,
    frontCardOverridesKey,
    backCardOverridesKey,
    flippedCards,
  ]);
  ...
  const cardControlLayouts = useMemo((): CardControlLayout[] => {
    ...
    return globalPixiCards
      .map((pixiCard, index) => {
        ...
        const isBlankBack =
          isFlipped && pixiCard.backImageId === "cardback_builtin_blank";
  ...
  return (
    <>
      <div
        className={`w-full h-full overflow-hidden bg-gray-200 dark:bg-gray-800 ${mobile ? "landscape:pl-6" : ""}`}
      >
        <PullToRefresh
          ref={scrollRef}
        ...
        >
          {cards.length === 0 ? (
            ...
            <div className="flex flex-col items-center justify-center h-full px-4 text-center select-none">
              <div
                className={`flex flex-col ${!mobile ? "md:flex-row" : ""} flex-wrap items-center justify-center gap-x-4 gap-y-3 mt-4 md:mt-0`}
              >
                <span
                  className={`text-3xl ${!mobile ? "sm:text-4xl md:text-5xl lg:text-7xl" : ""} font-bold text-gray-900 dark:text-white`}
                >
                  Welcome to
                </span>
                <img
                  ...
                  <DragOverlay zIndex={50}>
                    {activeId &&
                      (() => {
                        ...
                        if (multiDragState.current.isMultiDrag) {
                          const stackCards =
                            multiDragState.current.draggedCards;
                          ...
                          const others = stackCards
                            .filter((c) => c.uuid !== leaderId)
                            .slice(0, 2);
                          const leader = stackCards.find(
                            (c) => c.uuid === leaderId
                          );
                          const displayStack = leader
                            ? [...others, leader]
                            : others;
                            ...
                            <div className="relative">
                              {displayStack.map((card, index) => {
                                ...
                                const cardWidth =
                                  ...
                                  effectiveZoom;
                                const cardHeight =
                                  (baseCardHeightMm + bleedMm * 2) *
                                  MM_TO_PX *
                                  effectiveZoom;

                                const imageUrl = card.imageId
                                  ? processedImageUrls[card.imageId]
                                  : undefined;
                                ...
                                return (
                                  <div
                                    ...
                                    style={{
                                      ...
                                      transform: isTop
                                        ? "none"
                                        : `translate(${depth * 4}px, ${depth * 4}px) rotate(${depth * (index % 2 === 0 ? -2 : 2)}deg)`,
                                      top: 0,
                                      left: 0,
                                    }}
                                  >
                        ...
                        return (
                          ...
                          >
                            ...
                            <div
                              className={`absolute right-[4px] top-6 w-4 h-4 rounded-sm flex items-center justify-center z-20 ${isFlipped ? "bg-blue-500 text-white" : "bg-white text-gray-700"}`}
                            >
                              <svg
