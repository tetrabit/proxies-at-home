...
export interface MpcCalibrationDatasetRecord {
  id: string;
  name: string;
  description?: string;
...
export interface MpcCalibrationSourceCardSnapshot {
  cardUuid?: string;
  projectId?: string;
  imageId?: string;
  name: string;
...
export interface MpcCalibrationFrozenCandidate {
  identifier: string;
  name: string;
  rawName: string;
...
export interface MpcCalibrationComparisonHints {
  fullCard?: Record<string, number | null>;
  artMatch?: Record<string, number | null>;
}

export interface MpcCalibrationCaseRecord {
  id: string;
  datasetId: string;
  createdAt: number;
  updatedAt: number;
  source: MpcCalibrationSourceCardSnapshot;
  candidates: MpcCalibrationFrozenCandidate[];
  expectedIdentifier?: string;
  notes?: string;
  comparisonHints?: MpcCalibrationComparisonHints;
}

export type MpcCalibrationAssetRole =
  | "source"
  | "source-art"
  | "candidate-small";

export interface MpcCalibrationAssetRecord {
  id: string;
  datasetId: string;
  caseId: string;
...
export interface MpcCalibrationRunSummary {
  totalCases: number;
  matchedCases: number;
...
export interface MpcCalibrationRunRecord {
  id: string;
  datasetId: string;
  algorithmId: string;
...
export class ProxxiedDexie extends Dexie {
  ...
  mpcCalibrationDatasets!: Table<MpcCalibrationDatasetRecord, string>;
  ...
  constructor(databaseName = "ProxxiedDB") {
    ...
    this.version(6).stores({
      cards:
        "&uuid, imageId, order, name, needsEnrichment, linkedFrontId, linkedBackId",
      images:
        "&id, refCount, displayDpi, displayBleedWidth, exportDpi, exportBleedWidth",
      settings: "&id",
      imageCache: "&url, cachedAt",
    ...
    this.version(12).stores({
      cards:
        "&uuid, imageId, order, name, needsEnrichment, needs_token, linkedFrontId, linkedBackId",
      images:
        "&id, refCount, displayDpi, displayBleedWidth, exportDpi, exportBleedWidth",
      cardbacks: "&id",
      settings: "&id",
    ...
    this.version(16)
      .stores({
        cards:
          "&uuid, imageId, order, name, needsEnrichment, needs_token, linkedFrontId, linkedBackId, projectId",
        images:
          "&id, refCount, displayDpi, displayBleedWidth, exportDpi, exportBleedWidth",
        cardbacks: "&id",
        settings: "&id",
    ...
    // Version 20: Add MPC calibration harness tables
