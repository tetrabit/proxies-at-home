...
const mocks = vi.hoisted(() => ({
  ...
  addPage: vi.fn(() => ({ getWidth: () => 72, getHeight: () => 72, drawImage: vi.fn() })),
...
vi.mock('./effectCache', () => ({ getEffectCacheEntry: mocks.getEffectCacheEntry }));
vi.mock('./adjustmentUtils', () => ({ hasActiveAdjustments: mocks.hasActiveAdjustments }));
...
class MockWorker {
  ...
  postMessage = vi.fn((message) => {
    queueMicrotask(async () => {
      await this.onmessage?.({ data: { type: 'progress', pageIndex: message.pageIndex, imagesProcessed: message.pageCards.length } } as MessageEvent);
      await this.onmessage?.({ data: { type: 'result', pageIndex: message.pageIndex, url: `blob:page-${message.pageIndex}` } } as MessageEvent);
    });
  });
  constructor() {
...
describe('exportProxyPagesToPdf', () => {
  beforeEach(() => {
    ...
    vi.stubGlobal('fetch', vi.fn(async () => ({ blob: async () => new Blob(['jpg']) })));
    vi.spyOn(Blob.prototype, 'arrayBuffer').mockResolvedValue(new ArrayBuffer(3));
  ...
  it('honors maxPages zero with returnBuffer', async () => {
    const { exportProxyPagesToPdf } = await import('./exportProxyPageToPdf');
    const result = await exportProxyPagesToPdf({ cards: [{ uuid: 'c1', name: 'Card' }] as any, imagesById: new Map(), pdfSettings: baseSettings, pagesPerPdf: 1, maxPages: 0, cancellationPromise: new Promise(() => undefined), returnBuffer: true });
  ...
  it('renders worker pages, merges chunks, reports progress, and returns a buffer', async () => {
    ...
    const result = await exportProxyPagesToPdf({
      cards: [{ uuid: 'c1', name: 'One' }, { uuid: 'c2', name: 'Two', imageId: 'img', overrides: { brightness: 1 } }] as any,
      imagesById: new Map(),
  ...
  it('rejects image assembly failures and native worker errors', async () => {
    ...
    class EventWorker extends MockWorker {
      postMessage = vi.fn(() => queueMicrotask(() => this.onerror?.({} as ErrorEvent)));
    }
  ...
  it('rejects cancellation and worker errors while cleaning up workers', async () => {
    ...
    class ErrorWorker extends MockWorker {
      postMessage = vi.fn((message) => queueMicrotask(() => this.onmessage?.({ data: { error: 'bad', pageIndex: message.pageIndex } } as MessageEvent)));
    }
